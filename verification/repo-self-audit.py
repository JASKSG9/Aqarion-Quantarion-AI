...
def check_workflow(errors):
    p = ROOT / ".github" / "workflows" / "sv-001-v2.yaml"
    if not p.is_file():
        errors.append("WORKFLOW_MISSING:sv-001-v2.yaml")
        return
    ...
    if starts != ends:
        errors.append("WORKFLOW_UNTERMINATED_HEREDOC:sv-001-v2.yaml")
...
def check_receipt_policy(errors):
    ...
    for p in forbidden:
        if p.exists():
            errors.append(
                f"PREGENERATED_RECEIPT_FORBIDDEN:{p.relative_to(ROOT)}"
            )
