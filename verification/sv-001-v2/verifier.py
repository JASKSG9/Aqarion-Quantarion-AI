...
def matrix_verification(m: int, k: int, s: int) -> VerificationResult:
    ...
    if mutant == "wrong_trace":
        expected = type(expected)(m=expected.m, k=expected.k, s=expected.s,
                                  r=expected.r, alpha_squared=expected.alpha_squared,
                                  trace=expected.trace + 1.0, norm=expected.norm)
    elif mutant == "wrong_norm":
        expected = type(expected)(m=expected.m, k=expected.k, s=expected.s,
                                  r=expected.r, alpha_squared=expected.alpha_squared,
                                  trace=expected.trace, norm=expected.norm + 1.0)
    elif mutant == "wrong_gram":
        expected = type(expected)(m=expected.m, k=expected.k, s=expected.s,
                                  r=expected.r, alpha_squared=expected.alpha_squared * 2.0,
                                  trace=expected.trace, norm=expected.norm)
    elif mutant == "nan_error":
        expected = type(expected)(m=expected.m, k=expected.k, s=expected.s,
                                  r=expected.r, alpha_squared=expected.alpha_squared,
                                  trace=float("nan"), norm=expected.norm)
    ...
    finite = all(math.isfinite(v) for v in (gram_error, trace_error, norm_error))
    return VerificationResult(
        m=m, k=k, s=s, r=expected.r,
        gram_error=gram_error, trace_error=trace_error, norm_error=norm_error,
...
def run_all():
    failures = []
    max_gram = max_trace = max_norm = 0.0
    ...
    return {
        ...
        "failure_cases": [
            {"m": x.m, "k": x.k, "s": x.s, "r": x.r,
             "gram_error": x.gram_error, "trace_error": x.trace_error,
...
def main():
    ...
    result = run_all()
    print(f"SV-001-V2 matrix verifier cases={result['cases']} failures={result['failures']}")
